...
public class SearchAdapter extends BaseAdapter {
    ...
    private ArrayList<SearchModel> mSubcategories;

    public SearchAdapter(Context context, ArrayList<SearchModel> subcategories) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.otherText = (TextView) convertView.findViewById(R.id.search_other_text);
            convertView.setTag(holder);
        } else {
            holder = (ViewHolder) convertView.getTag();
        }

        SearchModel subcategory = mSubcategories.get(position);
        if (position == 0) { //We only show icon for the first item
            holder.icon.setVisibility(View.VISIBLE);
        } else {
            holder.icon.setVisibility(View.INVISIBLE);
        }
        holder.text.setText(subcategory.getText());
        holder.otherText.setText(subcategory.getOtherText());
